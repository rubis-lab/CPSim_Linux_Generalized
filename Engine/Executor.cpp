...
bool Executor::run_simulation(JobVectorOfSimulator& job_vector_of_simulator, JobVectorsForEachECU& job_vectors_for_each_ECU, double start_time)
{
    ...
    move_ecus_jobs_to_simulator(job_vector_of_simulator, job_vectors_for_each_ECU); 
    ...
    {
        // dedline miss
        if(job->get_actual_start_time() < 0 || job->get_actual_finish_time() > job->get_actual_deadline()) // Deadline missed
        if(job->get_actual_start_time() < 0 || job->get_actual_finish_time() > job->get_actual_deadline())  // missing deadline
        {
            utils::mtx_data_log.lock();
            global_object::logger->_201710233_real_cyber_event_logger((long long)job->get_actual_finish_time(), job->get_job_id(), "FINISHED (DEADLINE MISS)");
            utils::mtx_data_log.unlock();
            logger.real_cyber_event_logger_2017_14434(job->get_actual_deadline(), job->get_task_id(), "FINISHED (DEADLINE MISSED)");
            std::cout <<"DEADLINE MISS IN REAL CYBER SYSTEM" << std::endl;

            _2017_15782_real_cyber_event_logger(job->get_actual_finish_time(), job->get_task_id(),
                job->get_ECU()->get_num_of_task(), job->get_job_id(), "FINISHED (DEADLINE MISS)");
            logger._2017_13400_real_cyber_event_logger(job->get_actual_deadline(), job->get_task_id(), "FINISHED (DEADLINE MISSED)");
            int _log_job_id = std::stoi(std::to_string(job -> get_task_id() + 1) + std::to_string(job -> get_job_id()));
            global_object::logger -> _2018_11940_real_cyber_event_logger((long long)job->get_actual_deadline(), _log_job_id, "FINISHED (DEADLINE MISSED)");
            logger._2018_14000_real_cyber_event_logger(job->get_actual_deadline(), job->get_task_id(), "FINISHED (DEADLINE MISSED)");
        }
    }
    /**
    ...
    {   // until simulation finishes
            ...
            if((job->get_det_prdecessors().size() == 0) && ( job->get_is_simulated() == false) && (job->get_is_released() == false))
            {
                if(job->get_is_read())
                {
                    ...
                    else    // releasing job
                    {
                        ...
                        global_object::logger->_2020_90632_real_cyber_event_logger(job->get_actual_release_time(), job->get_task_id(), job->get_job_id(), "RELEASE");
                        logger.real_cyber_event_logger_2017_14434(job->get_simulated_release_time(), job->get_task_id(), "RELEASED");
                        logger._2018_14000_real_cyber_event_logger(job->get_simulated_release_time(), job->get_task_id(), "RELEASED");
                        simulation_ready_queue.push_back(job);
                        is_idle = false;

                        _2017_15782_real_cyber_event_logger(job->get_actual_release_time(),
                            job->get_task_id(), job->get_job_id(), job->get_ECU()->get_num_of_task(),
                            "RELEASED");
                        utils::mtx_data_log.lock();
                        global_object::logger->_201710233_real_cyber_event_logger((long long)job->get_actual_release_time(), job->get_job_id(), "RELEASED");
                        utils::mtx_data_log.unlock();
                        is_idle = false;   
                        logger._2017_13400_real_cyber_event_logger(job->get_simulated_release_time(), job->get_task_id(), "RELEASED");
                        // Log Job Release
                        int n = job->get_job_id();
                        int len = n ? 0:1; 
                        while (n) { len++, n/=10; }
                        global_object::logger->_2020_81520_real_cyber_event_logger(job->get_actual_release_time(), (job->get_task_id()+1)*(pow(10,len)) + job->get_job_id(), "RELEASED");   
                        int _log_job_id = std::stoi(std::to_string(job -> get_task_id() + 1) + std::to_string(job -> get_job_id()));
                        global_object::logger -> _2018_11940_real_cyber_event_logger((long long)job->get_actual_release_time(), _log_job_id, "RELEASED");
                    }
                }
                else   
                else    //releasing job
        ...
        else
        {
            ...
            logger.real_cyber_event_logger_2017_14434(run_job->get_simulated_start_time(), run_job->get_task_id(), "STARTED");
            ...
            int len = n ? 0:1; 
            while (n) { len++, n/=10; }
            global_object::logger->_2020_81520_real_cyber_event_logger(run_job->get_actual_start_time(), (run_job->get_task_id()+1)*(pow(10,len)) + run_job->get_job_id(), "STARTED");
            

            logger._2018_14000_real_cyber_event_logger(run_job->get_simulated_start_time(), run_job->get_task_id(), "STARTED"); // start running the job
            ...
            //     //return false;
            // }
            logger._2017_13400_real_cyber_event_logger(run_job->get_simulated_start_time(), run_job->get_task_id(), "STARTED"); // start running the job
            ...
            if (utils::real_workload)
            {
                ...
                global_object::logger->_201710233_real_cyber_event_logger((long long)run_job->get_actual_start_time(), run_job->get_job_id(), "STARTED");
                global_object::logger->_201710233_real_cyber_event_logger((long long)run_job->get_actual_finish_time(), run_job->get_job_id(), "FINISHED");
                ...
                int _log_job_id = std::stoi(std::to_string(run_job -> get_task_id() + 1) + std::to_string(run_job -> get_job_id()));
                global_object::logger -> _2018_11940_real_cyber_event_logger((long long)run_job->get_actual_start_time(), _log_job_id, "STARTED");
                ...
                {
                    global_object::logger->_2020_90632_real_cyber_event_logger(run_job->get_actual_deadline(), run_job->get_task_id(), run_job->get_job_id(), "FINISHED (DEADLINE MISS)");
                }
                else if(run_job->get_actual_finish_time() > run_job->get_actual_deadline() || run_job->get_actual_finish_time() < 0)
                {
                    global_object::logger->_2020_90632_real_cyber_event_logger(run_job->get_actual_start_time(), run_job->get_task_id(), run_job->get_job_id(), "STARTED");
                    global_object::logger->_2020_90632_real_cyber_event_logger(run_job->get_actual_deadline(), run_job->get_task_id(), run_job->get_job_id(), "FINISHED (DEADLINE MISS)");
                }
                else
                {
                    global_object::logger->_2020_90632_real_cyber_event_logger(run_job->get_actual_start_time(), run_job->get_task_id(), run_job->get_job_id(), "STARTED");
                    global_object::logger->_2020_90632_real_cyber_event_logger(run_job->get_actual_finish_time(), run_job->get_task_id(), run_job->get_job_id(), "FINISHED");
                }
                global_object::logger -> _2018_11940_real_cyber_event_logger((long long)run_job->get_actual_finish_time(), _log_job_id, "FINISHED");
                global_object::schedule_data.push_back(std::move(diagram_finish));
                utils::mtx_data_log.unlock();

                _2017_15782_real_cyber_event_logger(run_job->get_actual_start_time(),
                    run_job->get_task_id(), run_job->get_job_id(), run_job->get_ECU()->get_num_of_task(),
                    "STARTED");
                _2017_15782_real_cyber_event_logger(run_job->get_actual_finish_time(),
                    run_job->get_task_id(), run_job->get_job_id(), run_job->get_ECU()->get_num_of_task(),
                    "FINISHED");
                logger.real_cyber_event_logger_2017_14434(run_job->get_simulated_finish_time(), run_job->get_task_id(), "FINISHED");
            }
            else {
                utils::current_time += run_job->get_simulated_execution_time();
                logger.real_cyber_event_logger_2017_14434(utils::current_time, run_job->get_task_id(), "FINISHED");
            }
                        

                logger._2017_13400_real_cyber_event_logger(run_job->get_simulated_finish_time(), run_job->get_task_id(), "FINISHED");
            }
            else {
                utils::current_time += run_job->get_simulated_execution_time();
                logger._2017_13400_real_cyber_event_logger(utils::current_time, run_job->get_task_id(), "FINISHED");
                logger._2018_14000_real_cyber_event_logger(run_job->get_simulated_finish_time(), run_job->get_task_id(), "FINISHED");
            }
            else {
                utils::current_time += run_job->get_simulated_execution_time();
                logger._2018_14000_real_cyber_event_logger(utils::current_time, run_job->get_task_id(), "FINISHED");
            }
            
                if(run_job->get_actual_finish_time() > run_job->get_actual_deadline())
                {
                    // Log Job Finish (with missed deadline)
                    global_object::logger->_2020_81520_real_cyber_event_logger(run_job->get_actual_finish_time(), (run_job->get_task_id()+1)*(pow(10,len)) + run_job->get_job_id(), "FINISHED (DEADLINE MISSED)");
                }
                else{
                    // Log Job Finish
                    global_object::logger->_2020_81520_real_cyber_event_logger(run_job->get_actual_finish_time(), (run_job->get_task_id()+1)*(pow(10,len)) + run_job->get_job_id(), "FINISHED");
                }
            }
...
bool Executor::run_simulation(JobVectorOfSimulator& job_vector_of_simulator, JobVectorsForEachECU& job_vectors_for_each_ECU, double start_time)
{
    ...
    for(auto job : job_vector_of_simulator)
    {
        if(job->get_actual_start_time() < 0 || job->get_actual_finish_time() > job->get_actual_deadline()) // Deadline missed
        {
            ...
            // DEADLINE MISS event occurred
            global_object::logger->_2014_11561_real_cyber_event_logger((long long) job->get_actual_deadline(), job->get_task_id(), "FINISHED (DEADLINE MISS)");
            logger._2017_13400_real_cyber_event_logger(job->get_actual_deadline(), job->get_task_id(), "FINISHED (DEADLINE MISSED)");
        }
    }
    /**
    ...
    {   
            ...
            if((job->get_det_prdecessors().size() == 0) && ( job->get_is_simulated() == false) && (job->get_is_released() == false))
            {
                if(job->get_is_read())
                {
                    ...
                    else
                    {
                        ...
                        global_object::logger->_2014_11561_real_cyber_event_logger((long long) job->get_actual_release_time(), job->get_task_id(), "RELEASED");
                        is_idle = false;   
                        logger._2017_13400_real_cyber_event_logger(job->get_simulated_release_time(), job->get_task_id(), "RELEASED");
                    }
                }
        ...
        else
        {
            ...
            if (utils::real_workload)
            {
                ...
                run_job->set_is_simulated(true);
                run_job->run_function();
                // since run_job->get_is_simulated() is true, STARTED and FINISHED (after run_function()) events occurred
                global_object::logger->_2014_11561_real_cyber_event_logger((long long) run_job->get_actual_start_time(), run_job->get_task_id(), "STARTED");
                global_object::logger->_2014_11561_real_cyber_event_logger((long long) run_job->get_actual_finish_time(), run_job->get_task_id(), "FINISHED");
                utils::mtx_data_log.lock();
                std::shared_ptr<ScheduleData> diagram_start = std::make_shared<ScheduleData>(run_job->get_actual_start_time(), 0, std::to_string(run_job->get_actual_start_time()) + ", ECU" + std::to_string(run_job->get_ECU()->get_ECU_id()) + ": " + run_job->get_task_name() + ", 1\n");
                global_object::schedule_data.push_back(std::move(diagram_start));
